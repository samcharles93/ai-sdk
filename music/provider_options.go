...
// ProviderOptions map (typically [GenerateMusicRequest.ProviderOptions]) into
// a typed value.
//
// providerName is the key used to namespace the bucket — by convention the
// provider's [Provider.Name] return value, e.g. "minimax".
//
...
func ProviderOptionsFor[T any](po map[string]any, providerName string) (T, error) {
	...
	if err != nil {
		return zero, fmt.Errorf("music.ProviderOptionsFor[%T]: marshal: %w", zero, err)
	}
	var out T
	if err := json.Unmarshal(b, &out); err != nil {
		return zero, fmt.Errorf("music.ProviderOptionsFor[%T]: unmarshal: %w", zero, err)
	}
	return out, nil
}
