...
// the provider's [Provider.Name] return value, e.g. "togetherai", "cohere".
//
...
func ProviderOptionsFor[T any](po map[string]any, providerName string) (T, error) {
	...
	if err != nil {
		return zero, fmt.Errorf("rerank.ProviderOptionsFor[%T]: marshal: %w", zero, err)
	}
	var out T
	if err := json.Unmarshal(b, &out); err != nil {
		return zero, fmt.Errorf("rerank.ProviderOptionsFor[%T]: unmarshal: %w", zero, err)
	}
	return out, nil
}
