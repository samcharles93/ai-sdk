...
// GenerateVideo creates videos from a prompt by delegating to the
// underlying Provider. If the Client or its Provider is nil, it returns
// ErrNoProvider.
func (c *Client) GenerateVideo(ctx context.Context, req GenerateVideoRequest) (GenerateVideoResponse, error) {
	if c == nil || c.p == nil {
		return GenerateVideoResponse{}, ErrNoProvider
	}
	if req.Prompt == "" {
		return GenerateVideoResponse{}, ErrInvalidRequest
	}
	return c.p.GenerateVideo(ctx, req)
}
