...
// bytes of the audio data. This is a heuristic based on magic bytes.
func detectAudioFormat(data []byte) (extension string, mediaType string) {
	if len(data) < 12 {
		return "", ""
	}
	...
	// WebM: mka container (often used for audio)
	...
	if len(data) >= 12 {
		...
		if boxSize > 0 && string(data[4:8]) == "ftyp" {
