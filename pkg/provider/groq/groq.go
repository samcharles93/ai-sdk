...
// New returns a new Groq Provider. It returns an error if APIKey is empty.
func New(cfg Config) (*Provider, error) {
	if cfg.APIKey == "" {
		return nil, fmt.Errorf("groq: APIKey is required: %w", chat.ErrInvalidRequest)
	}
	base := cfg.BaseURL
	if base == "" {
		base = defaultBaseURL
...
// wireFunctionCall mirrors the Groq function call wire shape. Arguments
...
func (p *Provider) buildBody(req chat.Request, stream bool) (map[string]any, []chat.Warning, error) {
	if req.Model == "" {
		return nil, nil, fmt.Errorf("groq: model is required: %w", chat.ErrInvalidRequest)
	}
	if len(req.Messages) == 0 {
		return nil, nil, fmt.Errorf("groq: at least one message is required: %w", chat.ErrInvalidRequest)
	}
	...
	for i, m := range req.Messages {
		...
		// content types that Groq does not support as input.
		...
		for _, part := range m.GetParts() {
			...
			case chat.ImagePart:
				warnings = append(warnings, chat.Warning{
					...
					Message: "groq: ImagePart not supported by current models",
				})
			case chat.FilePart:
				warnings = append(warnings, chat.Warning{
					Type:    "unsupported-content",
					Message: "groq: FilePart not supported by current models",
				})
			case chat.ReasoningPart:
				// Groq does not have a reasoning_content wire field.
				// Append reasoning text to content for non-assistant
				// turns; for assistant turns warn and drop.
				if m.Role == chat.RoleAssistant {
					warnings = append(warnings, chat.Warning{
						Type:    "unsupported-content",
						Message: "groq: ReasoningPart on assistant input dropped; Groq has no reasoning replay mechanism",
					})
				} else if p.Text != "" {
					textChunks = append(textChunks, p.Text)
				}
		...
		case chat.RoleTool:
			if m.ToolCallID == "" {
				return nil, nil, fmt.Errorf("groq: tool message at index %d missing ToolCallID: %w", i, chat.ErrInvalidRequest)
			}
	...
	if len(req.Tools) > 0 {
		...
		body["tools"] = tools
		// Groq-specific: parallel_tool_calls defaults to true.
	...
	if req.ToolChoice != nil {
		...
		case chat.ToolChoiceTool:
			if req.ToolChoice.Name == "" {
				return nil, nil, fmt.Errorf("groq: tool_choice type=tool requires Name: %w", chat.ErrInvalidRequest)
			}
		...
		default:
			return nil, nil, fmt.Errorf("groq: unknown tool_choice type %q: %w", req.ToolChoice.Type, chat.ErrInvalidRequest)
		}
	}
...
func classifyHTTPError(resp *http.Response) error {
	...
	return fmt.Errorf("groq: status %d: %s: %w", resp.StatusCode, snippet, base)
}
...
func (p *Provider) Chat(ctx context.Context, req chat.Request) (chat.Response, error) {
	...
	if err != nil {
		return chat.Response{}, fmt.Errorf("groq: http do: %w", err)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&wr); err != nil {
		return chat.Response{}, fmt.Errorf("groq: decode response: %w", err)
	}
	out := chat.Response{
		ID:       wr.ID,
		Model:    wr.Model,
...
func (p *Provider) ChatStream(ctx context.Context, req chat.Request) (chat.Stream, error) {
	...
	resp, err := p.client.Do(httpReq)
	if err != nil {
		return nil, fmt.Errorf("groq: http do: %w", err)
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
...
type stream struct {
	...
	// (which Groq emits AFTER finish_reason when stream_options.
...
func (s *stream) Next(ctx context.Context) (chat.Chunk, error) {
	...
	for {
		...
		if len(line) == 0 && err != nil {
			...
			return chat.Chunk{}, fmt.Errorf("groq: stream read: %w", err)
		}
		...
		if jerr := json.Unmarshal(data, &ch); jerr != nil {
			return chat.Chunk{}, fmt.Errorf("groq: decode stream chunk: %w", jerr)
		}
		if ch.Usage != nil {
