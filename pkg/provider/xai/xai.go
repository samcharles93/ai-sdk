...
// Provider is a chat.Provider backed by the xAI chat completions API.
...
// New returns a new xAI Provider. It returns an error if APIKey is empty.
func New(cfg Config) (*Provider, error) {
	if cfg.APIKey == "" {
		return nil, fmt.Errorf("xai: APIKey is required: %w", chat.ErrInvalidRequest)
	}
	base := cfg.BaseURL
	if base == "" {
		base = defaultBaseURL
...
// Options carries xAI-specific request options wired via ProviderOptions.
type Options struct {
...
func (p *Provider) buildBody(req chat.Request, stream bool) (map[string]any, []chat.Warning, error) {
	if req.Model == "" {
		return nil, nil, fmt.Errorf("xai: model is required: %w", chat.ErrInvalidRequest)
	}
	if len(req.Messages) == 0 {
		return nil, nil, fmt.Errorf("xai: at least one message is required: %w", chat.ErrInvalidRequest)
	}
	...
	for i, m := range req.Messages {
		...
		for _, part := range m.GetParts() {
			...
			case chat.ImagePart:
				warnings = append(warnings, chat.Warning{
					...
					Message: "xai: ImagePart not supported by current models",
				})
			case chat.FilePart:
				warnings = append(warnings, chat.Warning{
					Type:    "unsupported-content",
					Message: "xai: FilePart not supported by current models",
				})
			case chat.ReasoningPart:
				if m.Role == chat.RoleAssistant {
					warnings = append(warnings, chat.Warning{
						Type:    "unsupported-content",
						Message: "xai: ReasoningPart on assistant input dropped; xAI has no reasoning replay mechanism",
					})
				} else if p.Text != "" {
					textChunks = append(textChunks, p.Text)
				}
		...
		case chat.RoleTool:
			if m.ToolCallID == "" {
				return nil, nil, fmt.Errorf("xai: tool message at index %d missing ToolCallID: %w", i, chat.ErrInvalidRequest)
			}
	...
	if req.ToolChoice != nil {
		...
		case chat.ToolChoiceTool:
			if req.ToolChoice.Name == "" {
				return nil, nil, fmt.Errorf("xai: tool_choice type=tool requires Name: %w", chat.ErrInvalidRequest)
			}
		...
		default:
			return nil, nil, fmt.Errorf("xai: unknown tool_choice type %q: %w", req.ToolChoice.Type, chat.ErrInvalidRequest)
		}
	}
	...
	opts, err := chat.ProviderOptionsFor[Options](req.ProviderOptions, "xai")
	if err != nil {
		return nil, nil, fmt.Errorf("xai: extract provider options: %w", err)
	}
...
func (p *Provider) Chat(ctx context.Context, req chat.Request) (chat.Response, error) {
	...
	if err != nil {
		return chat.Response{}, fmt.Errorf("xai: http do: %w", err)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&wr); err != nil {
		return chat.Response{}, fmt.Errorf("xai: decode response: %w", err)
	}
	out := chat.Response{
		ID:       wr.ID,
		Model:    wr.Model,
...
func (s *stream) Next(ctx context.Context) (chat.Chunk, error) {
	...
	for {
		...
		if len(line) == 0 && err != nil {
			...
			return chat.Chunk{}, fmt.Errorf("xai: stream read: %w", err)
		}
		...
		if jerr := json.Unmarshal(data, &ch); jerr != nil {
			return chat.Chunk{}, fmt.Errorf("xai: decode stream chunk: %w", jerr)
		}
		if ch.Usage != nil {
