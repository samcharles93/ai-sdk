...
- Speech input limits are enforced client-side where known: OpenAI defaults to 4096 characters and Groq Orpheus to 200, while the generic `openai-compatible` class is uncapped (Kokoro's limit is deployment-configurable). Override with `max_input_chars` in provider `options` or model `extra`; oversized text fails with `ErrInvalidRequest` before any request.
- Voice discovery is an optional capability: `speech.VoiceLister` (or `speech.Client.ListVoices`) enumerates voices for a model. The generic `openai-compatible` class queries the server's `/audio/voices` endpoint and normalises both the bare-string and object response shapes (Kokoro, speaches); the named `openai` class serves the documented static list; `groq` reports `ErrVoiceListingNotSupported` because its voices are model-scoped and not exposed by an API.
- Streaming synthesis is an optional capability: `speech.Streamer`, `speech.Client.StreamSpeech` and `Runtime.SpeechStream` yield `SpeechChunk`s (audio bytes, then a Done chunk carrying usage, then `io.EOF`). The named `openai` class streams via SSE for models that support it (`gpt-4o-mini-tts`) and raw chunked audio for `tts-1`/`tts-1-hd`. Other providers stay buffered and return `ErrStreamNotSupported`; the generic class leaves streaming disabled because self-hosted servers use provider-specific flags (Kokoro uses `stream: true`, not `stream_format`).
