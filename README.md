...
- Groq implements speech synthesis over its OpenAI-compatible `/openai/v1/audio/speech` endpoint (PlayAI and Orpheus models). Voices are model-scoped, so a request must set `Voice` explicitly; the SDK rejects a voice-less Groq request rather than guessing.
- Any self-hosted OpenAI-compatible TTS server (Kokoro-FastAPI, speaches, openedai-speech, LocalAI) resolves through the generic `openai-compatible` class with `base_url` set to the server and `auth.type: none` (for keyless servers). Switching TTS backends is then config-only, not a code change. Set `Voice` explicitly — the generic path inherits OpenAI's `alloy` default, and server voice IDs usually differ (for example Kokoro's `af_heart`).
