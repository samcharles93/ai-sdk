...
func TestChat_RateLimit_TypedProviderError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("X-Request-Id", "req-deepseek-123")
	...
	if perr.Provider != "deepseek" {
		t.Errorf("Provider = %q, want deepseek", perr.Provider)
	}
	if perr.StatusCode != http.StatusTooManyRequests {
		t.Errorf("StatusCode = %d, want %d", perr.StatusCode, http.StatusTooManyRequests)
	}
	if perr.RequestID != "req-deepseek-123" {
		t.Errorf("RequestID = %q, want req-deepseek-123", perr.RequestID)
	}
