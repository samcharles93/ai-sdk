// Package deepseek implements the chat.Provider interface for the DeepSeek
// chat completion API.
//
// DeepSeek exposes an OpenAI-compatible Chat Completions endpoint at
// https://api.deepseek.com/chat/completions. This package speaks that wire
// format directly using only the Go standard library.
package deepseek
...
type Config struct {
	...
	APIKey string
	// BaseURL overrides the API base URL. Defaults to https://api.deepseek.com.
	BaseURL string
	// HTTPClient overrides the HTTP client used for requests.
	HTTPClient *http.Client
}

// Provider is a chat.Provider backed by the DeepSeek chat completions API.
...
// New returns a new DeepSeek Provider. It returns an error if APIKey is empty.
func New(cfg Config) (*Provider, error) {
	if cfg.APIKey == "" {
		return nil, fmt.Errorf("deepseek: APIKey is required: %w", chat.ErrInvalidRequest)
	}
	base := cfg.BaseURL
	if base == "" {
		base = defaultBaseURL
...
type wireMessage struct {
	...
	// ReasoningContent is DeepSeek's `reasoning_content` field — the
	// chain-of-thought text emitted by the deepseek-reasoner model and
	// echoed back on assistant turns so the model can resume its prior
	// thinking. Empty for non-reasoner models.
...
func (p *Provider) buildBody(req chat.Request, stream bool) (map[string]any, []chat.Warning, error) {
	if req.Model == "" {
		return nil, nil, fmt.Errorf("deepseek: model is required: %w", chat.ErrInvalidRequest)
	}
	if len(req.Messages) == 0 {
		return nil, nil, fmt.Errorf("deepseek: at least one message is required: %w", chat.ErrInvalidRequest)
	}
	var warnings []chat.Warning
	msgs := make([]wireMessage, len(req.Messages))
	for i, m := range req.Messages {
		wm := wireMessage{Role: string(m.Role), Name: m.Name}
		// Walk canonical Parts, joining text and routing reasoning into
		// reasoning_content on assistant turns. Non-text content is not
		// supported by the deepseek-chat / deepseek-reasoner endpoints
		// today; warn and drop.
		...
		for _, part := range m.GetParts() {
			...
			case chat.ImagePart:
				warnings = append(warnings, chat.Warning{
					...
					Message: "deepseek: ImagePart not supported by current models",
				})
			case chat.FilePart:
				warnings = append(warnings, chat.Warning{
					Type:    "unsupported-content",
					Message: "deepseek: FilePart not supported by current models",
				})
		...
		case chat.RoleTool:
			if m.ToolCallID == "" {
				return nil, nil, fmt.Errorf("deepseek: tool message at index %d missing ToolCallID: %w", i, chat.ErrInvalidRequest)
			}
	...
	if req.ToolChoice != nil {
		...
		case chat.ToolChoiceTool:
			if req.ToolChoice.Name == "" {
				return nil, nil, fmt.Errorf("deepseek: tool_choice type=tool requires Name: %w", chat.ErrInvalidRequest)
			}
		...
		default:
			return nil, nil, fmt.Errorf("deepseek: unknown tool_choice type %q: %w", req.ToolChoice.Type, chat.ErrInvalidRequest)
		}
	}
...
func classifyHTTPError(resp *http.Response) error {
	...
	return fmt.Errorf("deepseek: status %d: %s: %w", resp.StatusCode, snippet, base)
}
...
func (p *Provider) Chat(ctx context.Context, req chat.Request) (chat.Response, error) {
	...
	if err != nil {
		return chat.Response{}, fmt.Errorf("deepseek: http do: %w", err)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&wr); err != nil {
		return chat.Response{}, fmt.Errorf("deepseek: decode response: %w", err)
	}
	out := chat.Response{
		ID:       wr.ID,
		Model:    wr.Model,
...
func (p *Provider) ChatStream(ctx context.Context, req chat.Request) (chat.Stream, error) {
	...
	resp, err := p.client.Do(httpReq)
	if err != nil {
		return nil, fmt.Errorf("deepseek: http do: %w", err)
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
...
type stream struct {
	...
	// (which DeepSeek emits AFTER finish_reason when stream_options.
...
func (s *stream) Next(ctx context.Context) (chat.Chunk, error) {
	...
	for {
		...
		if len(line) == 0 && err != nil {
			...
			return chat.Chunk{}, fmt.Errorf("deepseek: stream read: %w", err)
		}
		...
		if jerr := json.Unmarshal(data, &ch); jerr != nil {
			return chat.Chunk{}, fmt.Errorf("deepseek: decode stream chunk: %w", jerr)
		}
		if ch.Usage != nil {
