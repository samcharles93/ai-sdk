...
func TestChat_NonStreaming(t *testing.T) {
	...
	if gotBody["model"] != "grok-2" {
		t.Errorf("model: %v", gotBody["model"])
	}
	if _, ok := gotBody["temperature"]; !ok {
		t.Errorf("expected temperature in body")
	}
	if resp.ID != "resp-1" || resp.Model != "grok-2" {
...
func TestChatStream_SSE(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fl, _ := w.(http.Flusher)
		writes := []string{
			`data: {"id":"s1","model":"grok-2","choices":[{"index":0,"delta":{"role":"assistant","content":"He"}}]}` + "\n\n",
			`data: {"id":"s1","model":"grok-2","choices":[{"index":0,"delta":{"content":"llo"}}]}` + "\n\n",
			`: keepalive` + "\n\n",
			`data: {"id":"s1","model":"grok-2","choices":[{"index":0,"delta":{},"finish_reason":"stop"}]}` + "\n\n",
			`data: {"id":"s1","model":"grok-2","choices":[],"usage":{"prompt_tokens":4,"completion_tokens":2,"total_tokens":6}}` + "\n\n",
			`data: [DONE]` + "\n\n",
		}
...
func TestChat_ReasoningEffort(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = io.WriteString(w, `{
			"id":"resp-re",
			"model":"grok-2-reasoning",
			"choices":[{"index":0,"message":{"role":"assistant","content":"reasoned output"},"finish_reason":"stop"}],
	...
	resp, err := p.Chat(context.Background(), chat.Request{
		Model:    "grok-2-reasoning",
		Messages: []chat.Message{{Role: chat.RoleUser, Content: "complex problem"}},
		ProviderOptions: map[string]any{
			"xai": Options{ReasoningEffort: "high"},
		},
	})
	if err != nil {
		t.Fatalf("Chat: %v", err)
	}
	if resp.Content != "reasoned output" {
		t.Errorf("unexpected content: %q", resp.Content)
	}
	if gotBody["reasoning_effort"] != "high" {
		t.Errorf("reasoning_effort = %v", gotBody["reasoning_effort"])
	}
}
...
func TestChat_RateLimit_TypedProviderError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("X-Request-Id", "req-xai-123")
	...
	if perr.RequestID != "req-xai-123" {
		t.Errorf("RequestID = %q, want req-xai-123", perr.RequestID)
	}
