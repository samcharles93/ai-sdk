...
func TestChat_RateLimit_TypedProviderError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("X-Request-Id", "req-perplexity-123")
	...
	if perr.Provider != "perplexity" {
		t.Errorf("Provider = %q, want perplexity", perr.Provider)
	}
	if perr.StatusCode != http.StatusTooManyRequests {
		t.Errorf("StatusCode = %d, want %d", perr.StatusCode, http.StatusTooManyRequests)
	}
	if perr.RequestID != "req-perplexity-123" {
		t.Errorf("RequestID = %q, want req-perplexity-123", perr.RequestID)
	}
