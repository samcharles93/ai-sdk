...
// Config configures a Mistral Provider.
type Config struct {
	// APIKey is the Mistral API key. Required.
	APIKey string
	// BaseURL overrides the API base URL. Defaults to https://api.mistral.ai/v1.
	BaseURL string
	// HTTPClient overrides the HTTP client used for requests.
	HTTPClient *http.Client
}

// Provider is a chat.Provider and embed.Provider backed by the Mistral API.
...
// New returns a new Mistral Provider. It returns an error if APIKey is empty.
func New(cfg Config) (*Provider, error) {
	if cfg.APIKey == "" {
		return nil, fmt.Errorf("mistral: APIKey is required: %w", chat.ErrInvalidRequest)
	}
	base := cfg.BaseURL
	if base == "" {
		base = defaultBaseURL
...
// wireFunctionCall mirrors the Mistral function call wire shape. Arguments
...
func (p *Provider) buildBody(req chat.Request, stream bool) (map[string]any, []chat.Warning, error) {
	if req.Model == "" {
		return nil, nil, fmt.Errorf("mistral: model is required: %w", chat.ErrInvalidRequest)
	}
	if len(req.Messages) == 0 {
		return nil, nil, fmt.Errorf("mistral: at least one message is required: %w", chat.ErrInvalidRequest)
	}
	...
	for i, m := range req.Messages {
		...
		for _, part := range m.GetParts() {
			...
			case chat.ImagePart:
				warnings = append(warnings, chat.Warning{
					...
					Message: "mistral: ImagePart not supported",
				})
			case chat.FilePart:
				warnings = append(warnings, chat.Warning{
					Type:    "unsupported-content",
					Message: "mistral: FilePart not supported",
				})
			case chat.ReasoningPart:
				if m.Role == chat.RoleAssistant {
					warnings = append(warnings, chat.Warning{
						Type:    "unsupported-content",
						Message: "mistral: ReasoningPart on assistant input dropped; Mistral has no reasoning replay mechanism",
					})
				} else if p.Text != "" {
					textChunks = append(textChunks, p.Text)
				}
		...
		case chat.RoleTool:
			if m.ToolCallID == "" {
				return nil, nil, fmt.Errorf("mistral: tool message at index %d missing ToolCallID: %w", i, chat.ErrInvalidRequest)
			}
	...
	if req.ToolChoice != nil {
		...
		case chat.ToolChoiceTool:
			if req.ToolChoice.Name == "" {
				return nil, nil, fmt.Errorf("mistral: tool_choice type=tool requires Name: %w", chat.ErrInvalidRequest)
			}
		...
		default:
			return nil, nil, fmt.Errorf("mistral: unknown tool_choice type %q: %w", req.ToolChoice.Type, chat.ErrInvalidRequest)
		}
	}
...
func (p *Provider) newHTTPRequest(ctx context.Context, body map[string]any, path string) (*http.Request, error) {
...
func classifyHTTPError(resp *http.Response) error {
	...
	return fmt.Errorf("mistral: status %d: %s: %w", resp.StatusCode, snippet, base)
}
...
func (p *Provider) Chat(ctx context.Context, req chat.Request) (chat.Response, error) {
	...
	httpReq, err := p.newHTTPRequest(ctx, body, "/chat/completions")
	if err != nil {
		return chat.Response{}, err
	}
	resp, err := p.client.Do(httpReq)
	if err != nil {
		return chat.Response{}, fmt.Errorf("mistral: http do: %w", err)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&wr); err != nil {
		return chat.Response{}, fmt.Errorf("mistral: decode response: %w", err)
	}
	out := chat.Response{
		ID:       wr.ID,
		Model:    wr.Model,
...
func (p *Provider) ChatStream(ctx context.Context, req chat.Request) (chat.Stream, error) {
	...
	resp, err := p.client.Do(httpReq)
	if err != nil {
		return nil, fmt.Errorf("mistral: http do: %w", err)
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
...
type stream struct {
	...
	// (which Mistral emits AFTER finish_reason when stream_options.
...
func (s *stream) Next(ctx context.Context) (chat.Chunk, error) {
	...
	for {
		...
		if len(line) == 0 && err != nil {
			...
			return chat.Chunk{}, fmt.Errorf("mistral: stream read: %w", err)
		}
		...
		if jerr := json.Unmarshal(data, &ch); jerr != nil {
			return chat.Chunk{}, fmt.Errorf("mistral: decode stream chunk: %w", jerr)
		}
		if ch.Usage != nil {
...
func (p *Provider) Embed(ctx context.Context, req embed.Request) (embed.Response, error) {
	if req.Model == "" {
		return embed.Response{}, fmt.Errorf("mistral: model is required: %w", chat.ErrInvalidRequest)
	}
	if len(req.Inputs) == 0 {
		return embed.Response{}, fmt.Errorf("mistral: at least one input is required: %w", chat.ErrInvalidRequest)
	}
	body := map[string]any{
		"model": req.Model,
		"input": req.Inputs,
	}
	httpReq, err := p.newHTTPRequest(ctx, body, "/embeddings")
	if err != nil {
		return embed.Response{}, err
	}
	resp, err := p.client.Do(httpReq)
	if err != nil {
		return embed.Response{}, fmt.Errorf("mistral: http do: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
	...
	if err := json.NewDecoder(resp.Body).Decode(&wr); err != nil {
		return embed.Response{}, fmt.Errorf("mistral: decode embed response: %w", err)
	}
	embeddings := make([]embed.Embedding, len(wr.Data))
	for _, d := range wr.Data {
		if d.Index >= 0 && d.Index < len(embeddings) {
			embeddings[d.Index] = embed.Embedding{
				Index:  d.Index,
				Vector: d.Embedding,
			}
		}
